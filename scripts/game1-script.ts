...
for (let i = 0, currentX = 5; i < notelist.length; i++, currentX += 5) {
    if (notelist[i] == 0) {
        svgCode = svgCode + `<circle class="circle" cx="${currentX}" cy = "2.5" r="0.6" fill="var(--red)"/>`
    }
    else if (notelist[i] == 1) {
        svgCode = svgCode + `<circle class="circle" cx="${currentX}" cy = "6.5" r="0.6" fill="var(--red)"/>`
    }
    else {
        svgCode = svgCode + `<circle class="circle" cx="${currentX}" cy = "10.5" r="0.6" fill="var(--red)"/>`
    }
}
svgCode = svgCode + `</svg>`
// create static svg 
